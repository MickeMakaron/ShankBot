...
void SideBarWindowAssembler::assemble(const Frame& frame, const GuiParser::Data& gui, const TextParser::Data& text, const RectParser::Data& rect, const GuiSpriteParser::Data& guiSprite)
{
    ...
    try
    {
        ...
        auto deepestDialogIt = std::max_element(gui.misc.dialogs.begin(), gui.misc.dialogs.end());
        if(deepestDialogIt != gui.misc.dialogs.end())
        {
            while(!guiSprites.empty() && guiSprites.back().front()->order < *deepestDialogIt)
            {
                guiSprites.pop_back();
            }
        }
